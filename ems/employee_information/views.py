        ...
        try:
            ...
            if (data['id']).isnumeric() and int(data['id']) > 0:
                save_employee = Employees.objects.filter(id=data['id']).update(
                    ...
                    aadhaar_number=data['aadhaar_number'],  # Add Aadhaar number field
                    department_id=dept,
                    position_id=pos,
...
def download_employees_pdf(request):
    ...
    template_path = 'employee_information/employee_list_pdf.html'
    ...
    if pisa_status.err:
        return HttpResponse('An error occurred while creating the PDF: %s' % pisa_status.err)
...
def download_employee_pdf(request, employee_id):
    ...
    template_path = 'employee_information/employee_detail_pdf.html'
    context = {'employee': employee}

    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="{employee.firstname}{employee.middlename}{employee.lastname}.pdf"'
